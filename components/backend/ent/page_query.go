...
func (_q *PageQuery) QueryHackathon() *HackathonQuery {
...
func (_q *PageQuery) QueryPhases() *PhaseQuery {
...
func (_q *PageQuery) QueryModifier() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *PageQuery) WithHackathon(opts ...func(*HackathonQuery)) *PageQuery {
	query := (&HackathonClient{config: _q.config}).Query()
...
func (_q *PageQuery) WithPhases(opts ...func(*PhaseQuery)) *PageQuery {
	query := (&PhaseClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PageQuery) WithModifier(opts ...func(*UserQuery)) *PageQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Page, error) {
	...
	if query := _q.withHackathon; query != nil {
		if err := _q.loadHackathon(ctx, query, nodes,
			func(n *Page) { n.Edges.Hackathon = []*Hackathon{} },
			func(n *Page, e *Hackathon) { n.Edges.Hackathon = append(n.Edges.Hackathon, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPhases; query != nil {
		if err := _q.loadPhases(ctx, query, nodes,
			func(n *Page) { n.Edges.Phases = []*Phase{} },
			func(n *Page, e *Phase) { n.Edges.Phases = append(n.Edges.Phases, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withModifier; query != nil {
		if err := _q.loadModifier(ctx, query, nodes, nil,
			func(n *Page, e *User) { n.Edges.Modifier = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PageQuery) loadHackathon(ctx context.Context, query *HackathonQuery, nodes []*Page, init func(*Page), assign func(*Page, *Hackathon)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Page)
	nids := make(map[int]map[*Page]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(page.HackathonTable)
		s.Join(joinT).On(s.C(hackathon.FieldID), joinT.C(page.HackathonPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(page.HackathonPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(page.HackathonPrimaryKey[1]))
...
func (_q *PageQuery) loadPhases(ctx context.Context, query *PhaseQuery, nodes []*Page, init func(*Page), assign func(*Page, *Phase)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Page)
	nids := make(map[int]map[*Page]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(page.PhasesTable)
		s.Join(joinT).On(s.C(phase.FieldID), joinT.C(page.PhasesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(page.PhasesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(page.PhasesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *PageQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Page, init func(*Page), assign func(*Page, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_created_pages == nil {
			continue
		}
		fk := *nodes[i].user_created_pages
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_created_pages" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *PageQuery) loadModifier(ctx context.Context, query *UserQuery, nodes []*Page, init func(*Page), assign func(*Page, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Page)
	for i := range nodes {
		if nodes[i].user_modified_pages == nil {
			continue
		}
		fk := *nodes[i].user_modified_pages
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_modified_pages" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
