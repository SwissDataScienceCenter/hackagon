...
func (_q *PageQuery) QueryPhase() *PhaseQuery {
...
func (_q *PageQuery) WithPhase(opts ...func(*PhaseQuery)) *PageQuery {
	query := (&PhaseClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Page, error) {
	...
	if _q.withHackathon != nil || _q.withPhase != nil || _q.withCreator != nil || _q.withModifier != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withHackathon; query != nil {
		if err := _q.loadHackathon(ctx, query, nodes, nil,
			func(n *Page, e *Hackathon) { n.Edges.Hackathon = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPhase; query != nil {
		if err := _q.loadPhase(ctx, query, nodes, nil,
			func(n *Page, e *Phase) { n.Edges.Phase = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *PageQuery) loadHackathon(ctx context.Context, query *HackathonQuery, nodes []*Page, init func(*Page), assign func(*Page, *Hackathon)) error {
	...
	for i := range nodes {
		if nodes[i].hackathon_pages == nil {
			continue
		}
		fk := *nodes[i].hackathon_pages
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "hackathon_pages" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *PageQuery) loadPhase(ctx context.Context, query *PhaseQuery, nodes []*Page, init func(*Page), assign func(*Page, *Phase)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Page)
	for i := range nodes {
		if nodes[i].phase_page == nil {
			continue
		}
		fk := *nodes[i].phase_page
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "phase_page" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
