...
func (_q *QuestionQuery) QueryHackathon() *HackathonQuery {
...
func (_q *QuestionQuery) QueryCreator() *UserQuery {
...
func (_q *QuestionQuery) QueryModifier() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *QuestionQuery) WithHackathon(opts ...func(*HackathonQuery)) *QuestionQuery {
	query := (&HackathonClient{config: _q.config}).Query()
...
func (_q *QuestionQuery) WithCreator(opts ...func(*UserQuery)) *QuestionQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *QuestionQuery) WithModifier(opts ...func(*UserQuery)) *QuestionQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(question.FieldHackathonID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Question.Query().
//		Select(question.FieldHackathonID).
//		Scan(ctx, &v)
...
func (_q *QuestionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Question, error) {
	...
	if query := _q.withHackathon; query != nil {
		if err := _q.loadHackathon(ctx, query, nodes, nil,
			func(n *Question, e *Hackathon) { n.Edges.Hackathon = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCreator; query != nil {
		if err := _q.loadCreator(ctx, query, nodes, nil,
			func(n *Question, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withModifier; query != nil {
		if err := _q.loadModifier(ctx, query, nodes, nil,
			func(n *Question, e *User) { n.Edges.Modifier = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *QuestionQuery) loadHackathon(ctx context.Context, query *HackathonQuery, nodes []*Question, init func(*Question), assign func(*Question, *Hackathon)) error {
...
func (_q *QuestionQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Question, init func(*Question), assign func(*Question, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Question)
	for i := range nodes {
		if nodes[i].user_created_questions == nil {
			continue
		}
		fk := *nodes[i].user_created_questions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_created_questions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *QuestionQuery) loadModifier(ctx context.Context, query *UserQuery, nodes []*Question, init func(*Question), assign func(*Question, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Question)
	for i := range nodes {
		if nodes[i].user_modified_questions == nil {
			continue
		}
		fk := *nodes[i].user_modified_questions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_modified_questions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *QuestionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withHackathon != nil {
			_spec.Node.AddColumnOnce(question.FieldHackathonID)
		}
	}
