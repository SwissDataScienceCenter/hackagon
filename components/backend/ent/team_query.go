...
type TeamQuery struct {
	...
	withTeamParticipants *TeamParticipantQuery
	withFKs              bool
...
func (_q *TeamQuery) QueryProject() *ProjectQuery {
...
func (_q *TeamQuery) QueryCreator() *UserQuery {
...
func (_q *TeamQuery) QueryModifier() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *TeamQuery) QuerySubmissions() *SubmissionQuery {
...
// QueryTeamParticipants chains the current query on the "team_participants" edge.
func (_q *TeamQuery) QueryTeamParticipants() *TeamParticipantQuery {
...
func (_q *TeamQuery) Clone() *TeamQuery {
	...
	return &TeamQuery{
		...
		withTeamParticipants: _q.withTeamParticipants.Clone(),
...
func (_q *TeamQuery) WithProject(opts ...func(*ProjectQuery)) *TeamQuery {
	query := (&ProjectClient{config: _q.config}).Query()
...
func (_q *TeamQuery) WithCreator(opts ...func(*UserQuery)) *TeamQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) WithModifier(opts ...func(*UserQuery)) *TeamQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) WithSubmissions(opts ...func(*SubmissionQuery)) *TeamQuery {
	query := (&SubmissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithTeamParticipants tells the query-builder to eager-load the nodes that are connected to
// the "team_participants" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TeamQuery) WithTeamParticipants(opts ...func(*TeamParticipantQuery)) *TeamQuery {
	query := (&TeamParticipantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTeamParticipants = query
	return _q
}
...
func (_q *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	var (
		...
		loadedTypes = [6]bool{
			...
			_q.withMembers != nil,
			_q.withTeamParticipants != nil,
		}
	)
	if _q.withProject != nil || _q.withCreator != nil || _q.withModifier != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withProject; query != nil {
		if err := _q.loadProject(ctx, query, nodes, nil,
			func(n *Team, e *Project) { n.Edges.Project = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCreator; query != nil {
		if err := _q.loadCreator(ctx, query, nodes, nil,
			func(n *Team, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withModifier; query != nil {
		if err := _q.loadModifier(ctx, query, nodes, nil,
			func(n *Team, e *User) { n.Edges.Modifier = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSubmissions; query != nil {
		if err := _q.loadSubmissions(ctx, query, nodes,
			func(n *Team) { n.Edges.Submissions = []*Submission{} },
			func(n *Team, e *Submission) { n.Edges.Submissions = append(n.Edges.Submissions, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withTeamParticipants; query != nil {
		if err := _q.loadTeamParticipants(ctx, query, nodes,
			func(n *Team) { n.Edges.TeamParticipants = []*TeamParticipant{} },
			func(n *Team, e *TeamParticipant) { n.Edges.TeamParticipants = append(n.Edges.TeamParticipants, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TeamQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Team, init func(*Team), assign func(*Team, *Project)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Team)
	for i := range nodes {
		if nodes[i].project_teams == nil {
			continue
		}
		fk := *nodes[i].project_teams
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "project_teams" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TeamQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Team, init func(*Team), assign func(*Team, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Team)
	for i := range nodes {
...
func (_q *TeamQuery) loadModifier(ctx context.Context, query *UserQuery, nodes []*Team, init func(*Team), assign func(*Team, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Team)
	for i := range nodes {
		if nodes[i].user_modified_teams == nil {
			continue
		}
		fk := *nodes[i].user_modified_teams
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_modified_teams" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TeamQuery) loadSubmissions(ctx context.Context, query *SubmissionQuery, nodes []*Team, init func(*Team), assign func(*Team, *Submission)) error {
	...
	query.Where(predicate.Submission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.SubmissionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.team_submissions
		if fk == nil {
			return fmt.Errorf(`foreign-key "team_submissions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "team_submissions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *TeamQuery) loadTeamParticipants(ctx context.Context, query *TeamParticipantQuery, nodes []*Team, init func(*Team), assign func(*Team, *TeamParticipant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(teamparticipant.FieldTeamID)
	}
	query.Where(predicate.TeamParticipant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.TeamParticipantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
