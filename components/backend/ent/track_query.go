...
func (_q *TrackQuery) QueryHackathon() *HackathonQuery {
...
func (_q *TrackQuery) QueryProjects() *ProjectQuery {
...
func (_q *TrackQuery) QueryCreator() *UserQuery {
...
func (_q *TrackQuery) QueryModifier() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *TrackQuery) WithHackathon(opts ...func(*HackathonQuery)) *TrackQuery {
	query := (&HackathonClient{config: _q.config}).Query()
...
func (_q *TrackQuery) WithProjects(opts ...func(*ProjectQuery)) *TrackQuery {
	query := (&ProjectClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TrackQuery) WithCreator(opts ...func(*UserQuery)) *TrackQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TrackQuery) WithModifier(opts ...func(*UserQuery)) *TrackQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TrackQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Track, error) {
	...
	if query := _q.withHackathon; query != nil {
		if err := _q.loadHackathon(ctx, query, nodes, nil,
			func(n *Track, e *Hackathon) { n.Edges.Hackathon = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withProjects; query != nil {
		if err := _q.loadProjects(ctx, query, nodes,
			func(n *Track) { n.Edges.Projects = []*Project{} },
			func(n *Track, e *Project) { n.Edges.Projects = append(n.Edges.Projects, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCreator; query != nil {
		if err := _q.loadCreator(ctx, query, nodes, nil,
			func(n *Track, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withModifier; query != nil {
		if err := _q.loadModifier(ctx, query, nodes, nil,
			func(n *Track, e *User) { n.Edges.Modifier = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TrackQuery) loadHackathon(ctx context.Context, query *HackathonQuery, nodes []*Track, init func(*Track), assign func(*Track, *Hackathon)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Track)
	for i := range nodes {
		if nodes[i].hackathon_tracks == nil {
			continue
		}
		fk := *nodes[i].hackathon_tracks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "hackathon_tracks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TrackQuery) loadProjects(ctx context.Context, query *ProjectQuery, nodes []*Track, init func(*Track), assign func(*Track, *Project)) error {
	...
	query.Where(predicate.Project(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(track.ProjectsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.track_projects
		if fk == nil {
			return fmt.Errorf(`foreign-key "track_projects" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "track_projects" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *TrackQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Track, init func(*Track), assign func(*Track, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Track)
	for i := range nodes {
		if nodes[i].user_created_tracks == nil {
			continue
		}
		fk := *nodes[i].user_created_tracks
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_created_tracks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TrackQuery) loadModifier(ctx context.Context, query *UserQuery, nodes []*Track, init func(*Track), assign func(*Track, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Track)
	for i := range nodes {
		if nodes[i].user_modified_tracks == nil {
			continue
		}
		fk := *nodes[i].user_modified_tracks
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_modified_tracks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
