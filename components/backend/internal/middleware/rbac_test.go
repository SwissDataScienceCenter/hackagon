...
var _ = Describe("RBAC Enforcer", func() {
	...
	Describe("Role-based Access", func() {
		...
		DescribeTable(
			...
			Entry("alice owner reads h1 submissions", "alice", "h1", Submission, Read, true),
			Entry("alice owner cannot read h2 submissions", "alice", "h2", Submission, Read, false),
			Entry(
				"alice owner cannot write h1 submissions",
				"alice",
				"h1",
				Submission,
				Write,
