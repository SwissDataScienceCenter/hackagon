...
<!--
  Pen: pr-content padding [32,80]; inner column 960w; pr-list gap 8; header bottom 12; pagination top 20
-->
<div class="min-w-0 bg-white dark:bg-surface-950">
    <div class="px-4 py-8 sm:px-10 md:px-[80px]">
        <div class="mx-auto w-full max-w-[960px]">
        <header
            class="mb-0 flex w-full flex-col items-stretch justify-between gap-3
                   pb-3 sm:flex-row sm:items-center"
        >
            <h1
                class="shrink-0 font-mono text-[22px] font-bold leading-tight text-surface-950
            ...
            <a
                href="#propose"
                class="inline-flex h-9 w-full min-h-[36px] min-w-0 items-center
                       justify-center gap-1.5 rounded-none px-3 text-center
        ...
        {#if pageCount > 1}
            <nav
                class="mt-5 flex w-full justify-center gap-1"
            ...
            >
                {#each Array.from({ length: pageCount }, (_, i) => i + 1) as p (p)}
                    <button
                        type="button"
                        onclick={() => (page = p)}
                        class="flex h-8 w-8 items-center justify-center rounded-none text-xs
