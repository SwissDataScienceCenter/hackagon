<script lang="ts">
    ...
    const pageCount = $derived(Math.max(1, Math.ceil(data.proposals.length / pageSize)));
    const pagedProposals = $derived(
        data.proposals.slice((page - 1) * pageSize, page * pageSize)
    );
        ...
        {:else}
            {#each pagedProposals as proposal (proposal.id)}
                <ProposalCard
                    ...
                    moreInfoHref="#proposal-{proposal.id}"
                />
            {/each}
        {/if}
    </div>
