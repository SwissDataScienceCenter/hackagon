...
function formError(e: unknown): ActionFailure<VotingForm> {
  if (e instanceof ClientError) {
    if (e.code === Status.PERMISSION_DENIED)
      return bad(403, {
        message: e.details || "You are not allowed to do that.",
      })
    if (e.code === Status.UNAUTHENTICATED)
      return bad(401, { message: "Please sign in again." })
    if (e.code === Status.NOT_FOUND)
      return bad(404, { message: "That item no longer exists." })
    if (e.code === Status.ALREADY_EXISTS)
      return bad(409, { message: "That already exists." })
    if (e.code === Status.FAILED_PRECONDITION)
      return bad(409, {
        message: e.details || "That isn't possible right now.",
      })
    ...
    if (e.code === Status.UNIMPLEMENTED)
      return bad(501, {
        message: "This server does not run the voting service yet.",
      })
  }
  throw e
}
...
function rememberBallot(cookies: Cookies, categoryId: string, voteId: string) {
  const entries = Object.entries(readBallots(cookies)).filter(
    ([k]) => k !== categoryId,
  )
  entries.push([categoryId, voteId])
  cookies.set(
    BALLOT_COOKIE,
    JSON.stringify(Object.fromEntries(entries.slice(-BALLOT_COOKIE_MAX))),
    {
      path: "/",
      httpOnly: true,
      sameSite: "lax",
...
export const load: PageServerLoad = async (event) => {
  ...
  try {
    ...
    if (e instanceof ClientError && e.code === Status.UNIMPLEMENTED)
  ...
  const labelFor = (id: string) =>
    submissionLabels.get(id) ?? "Unknown submission"
  ...
  async function myBallotFor(categoryId: string) {
    ...
    try {
      ...
      if (!ballot || ballot.voterId !== myUserId)
        return { cast: false, submissionId: "" }

      return {
        cast: true,
        submissionId: ballot.singleChoice?.submissionId ?? "",
      }
    } catch (e) {
  ...
  const detailed = await Promise.all(
    categories.map(async (c) => {
      ...
      return {
        ...
        myVoteLabel: myBallot.submissionId
          ? labelFor(myBallot.submissionId)
          : "",
      }
    }),
  )
...
export const actions: Actions = {
  ...
  createCategory: async (event) => {
    ...
    if (name.length < 3)
      return bad(400, { message: "A category name needs three characters." })
    ...
    try {
      await vote.createVoteCategory({
        ...
        maxPoints:
          votingMethod === METHOD_POINTS ? maxPointsFrom(form) : undefined,
  ...
  castBallot: async (event) => {
    ...
    if (method === METHOD_RANKED) {
      ...
      if (ranks.some((r) => !Number.isInteger(r) || r < 1))
        return bad(400, {
          message: "Give every submission a rank of 1 or more.",
        })
      ballot = {
        ranked: {
          categoryId,
      ...
      if (!submissionId)
        return bad(400, { message: "Pick a submission first." })
    ...
    try {
      ...
      if (e instanceof ClientError && e.code === Status.ALREADY_EXISTS) {
        return bad(409, {
          ...
          message:
            "You have already voted in this category. Ballots are final.",
        })
      }
  ...
  createResult: async (event) => {
    ...
    if (!categoryId) return bad(400, { message: "Missing category." })
    if (!submissionId)
      return bad(400, { message: "Pick the submission to place." })
  ...
  exportVotes: async (event) => {
    ...
    let res
    try {
      res = await vote.exportVotes({
        categoryId,
        format: json ? EXPORT_JSON : EXPORT_CSV,
      })
    } catch (e) {
      return formError(e)
    }
