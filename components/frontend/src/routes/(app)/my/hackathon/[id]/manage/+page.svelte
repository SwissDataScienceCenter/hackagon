<script lang="ts">
    ...
    import { canEditHackathon, manageNav } from '$lib/navigation';
    ...
    // these tiles too and is never described in two places. Pinned by
    // `tiles.test.ts`, which adds an entry to `manageNav` and requires it here
    ...
    // same `AdvancePhase`; what differs is what is actually being offered,
    ...
    const phaseAction = $derived.by(() => {
        if (data.declared) {
            return data.nextPhase
                ? { phaseId: data.nextPhase.id, label: `Advance to ${data.nextPhase.name}` }
                : undefined;
        }
        if (data.currentPhase) {
            return {
                phaseId: data.currentPhase.id,
                label: `Declare ${data.currentPhase.name} current`,
            };
        }
        return data.nextPhase
            ? { phaseId: data.nextPhase.id, label: `Start ${data.nextPhase.name}` }
            : undefined;
    });
</script>

<svelte:head><title>Manage · {data.hackathonName}</title></svelte:head>
...
<!--
  ...
  capability switches behind the participant timeline.
...
<div class="flex w-full flex-col gap-6 px-4 py-8 sm:px-10 md:px-20">
    ...
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="flex min-w-0 flex-col gap-1">
            <h1 class="m-0 text-title text-ink">Manage Hackathon</h1>
            <span class="text-xs text-ink-3">
                What participants may do, and where {data.hackathonName} is now.
            </span>
        </div>
        <div class="flex flex-wrap items-center gap-2">
            <!-- Approving is the organiser action most easily forgotten, because
                 nothing about a waitlisted participant is visible from anywhere
                 else. It rides here rather than on a tile because our
                 participants screen is one page for both roles — there is no
                 "Manage Participants" entry in `manageNav` to badge, and adding
                 one pointing at the same URL as the member entry is the
                 duplicate-meaning bug this nav already fixed once. Warning
                 variant, the same one the sidebar uses for "something needs
                 you"; absent entirely when nobody is waiting, so it is a
                 prompt and never a permanent readout. -->
            {#if data.waitingCount > 0}
                <a
                    href={resolve(`/my/hackathon/${data.hackathonId}/participants`)}
                    class="btn btn-sm btn-warning no-underline"
                >
                    <UserRoundCheck class="h-3 w-3 shrink-0" aria-hidden="true" />
                    Review {data.waitingCount} waiting
                </a>
            {/if}
            {#if mayEdit}
                <a
    ...
    <!-- The phase marker. Its own box above the switches, because the two are
         genuinely independent as far as this page is concerned: the button moves
         the marker, and what participants are allowed to do is the panel below.
    ...
    <section class="card flex flex-col gap-3 border-line-strong px-5 py-4">
        ...
        <div class="grid gap-2 sm:grid-cols-2">
            <div class="card card-raised flex flex-col gap-1 px-4 py-3">
                <div class="flex flex-wrap items-baseline justify-between gap-2">
                    ...
                    <!-- Declared, or merely running by the calendar. Named apart
                         ...
                         cleared. -->
                    {#if data.currentPhase}
                        <span class="badge {data.declared ? 'badge-info' : 'badge-neutral'}">
                            {data.declared ? 'Declared' : 'By dates'}
                        </span>
                    {/if}
                </div>
                {#if data.currentPhase}
                    <h2 class="m-0 text-section text-ink">{data.currentPhase.name}</h2>
                    <span class="tnum text-xs text-accent-ink">
                        {formatPhaseRange(data.currentPhase.startsAt, data.currentPhase.endsAt)}
                    </span>
                {:else}
            ...
            <div class="card card-raised flex flex-col gap-1 px-4 py-3">
                ...
                {#if data.nextPhase}
                    <h2 class="m-0 text-section text-ink-2">{data.nextPhase.name}</h2>
                    <span class="tnum text-xs text-ink-3">
                        {formatPhaseRange(data.nextPhase.startsAt, data.nextPhase.endsAt)}
                    </span>
                {:else}
                    <p class="m-0 text-sm text-ink-3">Nothing after this</p>
                    <span class="text-xs text-ink-3">
                        Add a phase on the timeline to carry on.
                    </span>
                {/if}
            </div>
        </div>
        ...
        <!-- On this branch advancing DOES move the switches a phase names as its
             opening one — `AdvancePhase` writes them in the same transaction. The
             ones no phase is linked to are the panel's alone, which is the part
             worth stating where the button is. -->
        <p class="m-0 text-xs text-ink-3">
            Moving the marker also applies what the phase it lands on opens. Anything no phase
            names stays exactly as the panel below leaves it.
        </p>
    </section>

    <!-- The switches, and the plan-vs-reality warning that belongs beside them
         because the fix is one of them. Moved here from the timeline, which is
         the participant-facing phase list on this branch: hackathon-wide
         configuration was sitting above a list of phases it is not one of. -->
    <CapabilitiesPanel
        currentPhaseName={data.currentPhase?.name ?? ''}
        ...
        <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {#each shortcuts as item (item.id)}
                ...
                <!-- eslint-disable svelte/no-navigation-without-resolve -- built with
                     resolve() in $lib/navigation; the rule only recognizes a
                     literal resolve() call in the attribute itself. -->
                <a
                    href={item.href}
                    class="card group flex items-start gap-3 px-4 py-3 no-underline
                ...
                >
                    ...
                    <span class="flex min-w-0 flex-1 flex-col gap-0.5">
                        <span class="text-sm leading-snug font-semibold text-ink">
                            {item.label}
                        </span>
                        <!-- Described once, in `manageNav`, so a destination
                             explained here and in the dashboard's platform tiles
                             cannot end up with two descriptions. -->
                        {#if item.description}
                            <span class="text-meta text-ink-3">{item.description}</span>
                        {/if}
                    </span>
                    <!-- The entry's own state chip, which the sidebar rows
                         already rendered and these tiles used to drop. -->
                    {#if item.badge}
                    ...
                    {/if}
                    <ArrowRight
                        class="mt-0.5 h-4 w-4 shrink-0 text-ink-3 transition-transform
