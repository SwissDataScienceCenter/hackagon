<script lang="ts">
    ...
    import { platformNav, type NavItem } from '$lib/navigation';
    ...
    interface Props {
         ...
         *
         * Gates the platform administration section as a whole — the settings
         * pages behind it need exactly this role, `UserService.List` denying
         * everyone else. Whole section rather than per tile, since every entry
         * in it needs the same role.
         *
         * It used to gate a second thing, the per-row edit pencil. That link
         * moved to Manage Hackathon, so nothing on this page decides who may
         * edit a hackathon any more.
         */
                    ...
                    {#each myHackathons as h, i (h.id)}
                        ...
                        <div class="flex items-center">
                            ...
                            <!-- The row's only trailing element. Editing a
                                 ...
                                 page now, not from here: this list is for finding
                                 a hackathon, and the edit form belongs beside the
