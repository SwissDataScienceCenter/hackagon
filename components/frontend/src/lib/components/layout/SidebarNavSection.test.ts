...
// Explicit, like MarkdownEditor.test.ts: every case here renders its own
// section, and two left mounted would make `getAllByRole("link")` count both.
afterEach(cleanup)

// A real icon rather than a stub: the component renders it as `<Icon />`, and
// the ComponentType/Component mismatch this codebase works around (see the note
// at the top of $lib/navigation) is exactly the kind of thing a stub would paper
// over.
...
describe("SidebarNavSection", () => {
  ...
  describe("when collapsed to the icon rail", () => {
    ...
    it("drops the label text but keeps it as the link's title", () => {
      render(SidebarNavSection, { items: [item("teams")], collapsed: true })
