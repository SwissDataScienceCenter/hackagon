        ...
        <div class="grid gap-10 py-12 sm:grid-cols-2 md:grid-cols-4">
            ...
            <nav class="flex flex-col gap-3" aria-label="Platform">
                ...
                <!-- "Hackathons" is the browse page — every event, searchable —
                     not home: the same word meant two things when it pointed at
                     the dashboard while signed in (see NavBar). -->
    ...
    <div class="border-t border-line px-4 sm:px-10 md:px-20">
        <div class="flex flex-col gap-4 py-6 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-x-5 gap-y-3">
                <!-- Our own pages, not datascience.ch's: this platform's privacy
                     and terms can differ from SDSC's corporate ones, and both
                     slugs are real SitePage routes here. -->
                <nav class="flex flex-wrap items-center gap-x-4 gap-y-2" aria-label="Legal">
                    <!-- eslint-disable-next-line svelte/no-navigation-without-resolve -- SitePage slug, not a typed route -->
                    <a href="/privacy" class={LEGAL}>Privacy</a>
                    <!-- eslint-disable-next-line svelte/no-navigation-without-resolve -- SitePage slug, not a typed route -->
                    <a href="/terms" class={LEGAL}>Terms of use</a>
                </nav>
