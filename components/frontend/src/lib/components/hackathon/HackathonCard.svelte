...
<a
    {href}
    class="card preset-outlined-surface-200-800 bg-surface-50-950 group flex flex-col overflow-hidden
...
>
    ...
    >
        ...
        {/if}
        {#if badge}
            <span class="badge {badgePreset} absolute top-2 right-2 text-xs">{badge}</span>
        {/if}
    </div>

    <div class="flex flex-1 flex-col gap-2 p-4">
        <h3 class="font-bold leading-snug group-hover:text-primary-500">{name}</h3>

        {#if meta}
            <p class="flex items-center gap-1.5 text-xs text-surface-500">
        ...
        {#if description}
            <!-- Clamped so panels in a row stay the same height whatever the
                 organizer wrote. -->
            <p class="line-clamp-3 text-sm text-surface-700-300">{description}</p>
        {/if}
    </div>
</a>
