...
<div class="flex flex-col gap-3">
    <div class="flex flex-wrap items-center gap-2 text-sm">
        <span class="badge badge-info">{addresses.length} recipient{addresses.length === 1 ? '' : 's'}</span>
        {#if audienceLabel}<span class="text-ink-3">{audienceLabel}</span>{/if}
    ...
    {#if unresolved.length > 0}
        <p class="card border-warning/40 p-3 text-xs text-warning-ink">
    ...
    {:else}
        <div class="flex flex-wrap gap-2">
            ...
            {/if}
            <button class="btn btn-sm" onclick={() => copy('to', addresses.join(', '))}>
                {copied === 'to' ? 'Copied' : 'Copy recipients'}
            </button>
            <button class="btn btn-sm" onclick={() => copy('subject', filledSubject)}>
                {copied === 'subject' ? 'Copied' : 'Copy subject'}
            </button>
            <button class="btn btn-sm" onclick={() => copy('body', filledBody)}>
        ...
        <details class="card p-3">
            ...
            <dl class="mt-3 flex flex-col gap-2 text-sm">
                <dt class="text-xs font-semibold text-ink-3">BCC</dt>
                <dd class="max-h-24 overflow-y-auto break-all font-mono text-xs">
                    {addresses.join(', ')}
                </dd>
                <dt class="text-xs font-semibold text-ink-3">Subject</dt>
                <dd>{filledSubject || '(no subject set)'}</dd>
                <dt class="text-xs font-semibold text-ink-3">Message</dt>
