...
<details class="relative inline-block text-left" bind:this={menuEl} ontoggle={syncOpen}>
    ...
    <summary
        ...
        class="flex h-8 w-8 cursor-pointer list-none items-center justify-center rounded
               hover:bg-surface-200-800 [&::-webkit-details-marker]:hidden [&::marker]:content-none"
    >
    ...
    <div
        role="menu"
        class="card preset-outlined-surface-200-800 bg-surface-50-950 absolute right-0 z-50 mt-1
               flex w-max min-w-40 flex-col py-1 text-left text-sm shadow-xl
               [&_button]:px-3 [&_button]:py-1.5 [&_button]:text-left [&_button]:hover:bg-surface-100-900
               [&_a]:px-3 [&_a]:py-1.5 [&_a]:no-underline [&_a]:hover:bg-surface-100-900"
    >
        {@render children()}
    </div>
</details>
