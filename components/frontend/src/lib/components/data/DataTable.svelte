...
{#if rows.length === 0}
    <p class="py-6 text-center text-sm text-ink-3">{empty}</p>
{:else}
        ...
        <table class="w-full border-collapse text-sm">
            ...
            <tbody>
                {#each sorted as r (rowKey(r))}
                    <tr class="border-t border-line hover:bg-raised">
                        {@render row(r)}
                    </tr>
                {/each}
            </tbody>
