...
<section class="card preset-outlined-surface-200-800 flex flex-col gap-3 p-4">
    <div class="flex flex-col gap-1">
        <div class="flex flex-wrap items-center gap-2">
            <h3 class="text-lg font-semibold">{category.name}</h3>
            <span class="badge preset-tonal">{category.methodLabel}</span>
            <span class="badge preset-tonal-surface">{category.voterTypeLabel}</span>
        </div>
    ...
    {#if decided}
        <div class="preset-tonal-success rounded p-3 text-sm">
    ...
    {:else}
        {#if !votingOpen}
            <p class="preset-tonal-warning rounded p-3 text-sm">
                ...
                {#each submissions as s (s.id)}
                    <label class="flex items-start gap-2">
                        ...
                        <span class="min-w-0">
                            ...
                            <span class="block text-xs text-surface-500">{s.status}</span>
                        </span>
                    </label>
                {/each}
            </fieldset>
            <div>
                <button class="btn btn-sm preset-filled-primary-500">Cast ballot</button>
            </div>
        </form>
    {/if}
</section>
