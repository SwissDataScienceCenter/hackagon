...
import type { CapabilityStatus } from "$lib/server/grpc/generated/hackathon/entities/capability"
 ...
 *
 * Adapted from main's model: there a hackathon carried a `state` row whose
 * capabilities were booleans. Here `Hackathon.capabilities` is a
 * `CapabilityStatus[]` whose `state` is the four-state answer the server
 * computes — COMING / OPEN / CLOSED / UNGOVERNED — so "switched on" means OPEN
 * rather than `enabled: true`. UNGOVERNED deliberately does not count: no row
 * governs it, so nothing is enforced and nothing was turned on.
 */
export function enabledCapabilities(
  ...
  return (capabilities ?? [])
    .filter((c) => (c.state as number) === OPEN)
