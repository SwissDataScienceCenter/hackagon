...
holds a manifest of deliberate, reversible breakages, each paired with the
tests that MUST notice; `scripts/mutate.sh` applies one, runs them, and asserts
...
The fast tier drives `go test` and `vitest` straight from source, so it needs
no running stack and never enters `nix develop` (`.devenv/profile/bin` has the
