...
command -v jq >/dev/null || { echo "error: jq not found (run inside the Nix dev shell)" >&2; exit 1; }
...
for f in "${FILES[@]}"; do
  ...
  [ "$n" -eq 0 ] && { echo "── $slug: no images"; continue; }
  echo "── $slug ($n)"
  mkdir -p "$STATIC/$slug"
  ...
  for i in $(seq 0 $((n - 1))); do
    ...
    if [ -f "$dest" ] && [ "$FORCE" -eq 0 ]; then
      echo "   [=] $file"; skipped=$((skipped + 1)); continue
    fi

    case "$src" in
      repo:*)
        srcpath="$ROOT_DIR/${src#repo:}"
        if [ -f "$srcpath" ] && place "$srcpath" "$dest"; then echo "   [c] $file  ← repo"
        else echo "   ✕ missing or unconvertible repo file: $srcpath"; failed=$((failed + 1)); continue; fi
        ;;
      http*)
        tmp=$(mktemp)
        if curl -fsSL -A "hackagon-seed/1.0 (SDSC archive)" -o "$tmp" "$src" && place "$tmp" "$dest"; then
          ...
          echo "   ✕ download failed: $src"; failed=$((failed + 1)); rm -f "$tmp" "$dest"; continue
        fi
        ;;
      *) echo "   ✕ unknown source scheme: $src"; failed=$((failed + 1)); continue ;;
    esac
    got=$((got + 1))
  done
done
