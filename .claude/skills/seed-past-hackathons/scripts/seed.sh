...
candidate_bases() {
  [ -n "$OBJECTS_BASE" ] && { echo "$OBJECTS_BASE"; return; }
...
upload() {
  ...
  for b in $(candidate_bases); do
    ...
    if [ "$code" = "200" ] || [ "$code" = "204" ]; then
      [ -z "$OBJECTS_BASE" ] && { OBJECTS_BASE="$b"; echo "   [i] uploading via $b" >&2; }
      echo "$pub"
      return 0
    fi
  done
...
for f in "${FILES[@]}"; do
  ...
  if grep -Fxq "$name" <<<"$existing"; then
    ...
    [ -z "$hid" ] && { echo "   ✕ present but not findable by name" >&2; continue; }
    FRESH=0
    echo "   [~] refreshing $hid"
  ...
  for i in $(seq 0 $((n - 1))); do
    ...
    src="$MEDIA_DIR/$file"
    [ -f "$src" ] || { echo "   [~] $file missing from static/$slug — run fetch-media.sh" >&2; continue; }
