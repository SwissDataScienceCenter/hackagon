...
for f in "$SKILL_DIR"/data/*.json; do
  echo "── $(basename "$f")"
  jq empty "$f" 2>/dev/null || { echo "   ✕ invalid JSON"; fail=1; continue; }

  slug=$(jq -r '.slug // empty' "$f")
  for field in slug name visibility startsAt endsAt description; do
    v=$(jq -r --arg k "$field" '.[$k] // empty' "$f")
    [ -z "$v" ] && { echo "   ✕ missing required field: $field"; fail=1; }
  done

  s=$(jq -r .startsAt "$f"); e=$(jq -r .endsAt "$f")
  if ! date -d "$s" >/dev/null 2>&1 || ! date -d "$e" >/dev/null 2>&1; then
    echo "   ✕ unparseable dates: $s / $e"; fail=1
  ...
  for i in $(seq 0 $((n - 1))); do
    ...
    if [ -z "$file" ] || [ -z "$src" ]; then
      echo "   ✕ media[$i] needs both 'file' and 'source'"; fail=1; continue
    fi
    if [ -f "$STATIC/$slug/$file" ]; then
      echo "   [x] $slug/$file ($(du -h "$STATIC/$slug/$file" | cut -f1))"
    else
      echo "   ✕ not fetched: static/$slug/$file — run scripts/fetch-media.sh"; fail=1
    fi
  done
...
echo ""
if [ "$fail" -eq 0 ]; then echo "All editions valid."; else echo "Validation failed."; exit 1; fi
