...
badge() {
  ...
  resp=$(rpc storage.StorageService/CreateUploadUrl "$req" 2>/dev/null) || { echo ""; return 0; }
  ...
  if [ "$code" != "200" ] && [ "$code" != "204" ]; then echo ""; return 0; fi
  echo "$pub"
}
...
for f in "$SKILL_DIR"/data/*.json; do
  ...
  [ -z "$hid" ] && { echo "── $name — not seeded, skipping"; continue; }
  echo "── $name"
  if [ "$DRY" -eq 1 ]; then echo "   [dry-run] would set 4 prizes with images"; continue; fi
