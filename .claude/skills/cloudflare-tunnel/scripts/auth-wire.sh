...
# components/frontend/src/lib/server/settings.ts). --restore deletes it.
...
if [ "${1:-}" = "--restore" ]; then
  ...
  # Restore is a DELETE now: the overlay is the only thing wiring wrote, and
  # with it gone both loaders fall back to the tracked config.yaml on their
  # next boot. Nothing to diff, nothing to reconcile, and no way for a stale
  # backup to overwrite an edit somebody made to config.yaml meanwhile.
  restored=0
  ...
  if [ "$restored" = 1 ]; then
    ...
    echo "Nothing to restore (no config.local.yaml — already on localhost)."
  fi
  exit 0
fi
...
write_overlay() { # <path> <yaml-body>
  cat >"$1" <<EOF
# GENERATED by .claude/skills/cloudflare-tunnel/scripts/auth-wire.sh — do not
# edit, do not commit (gitignored). Deleted by \`auth-wire.sh --restore\`,
# which down.sh calls for you.
#
# Overlays config.yaml: read after it (and, on the backend, before the
# HACKAGON_* env vars). Only the key below differs from the tracked config; a
# Cloudflare quick-tunnel hostname is this machine's for the next few hours
