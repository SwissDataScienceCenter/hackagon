...
public and private, with teams, draft/final submissions and a waitlisted person.
Dates slide with the seed run, so they will not match yours exactly.
...
> hand: `DOCS_SHOTS=1 pnpm exec playwright test --project=docs` writes straight
> into `docs/flows/`. The suite self-skips without `DOCS_SHOTS=1`, so a normal
> test run never rewrites committed images. Theme and viewports (1440×900 /
> 390×844) are pinned in the spec so a re-take cannot silently change the whole
> set.
...
and the event's own News & Pages. Private events are not listed here at all, and
a guessed URL returns the same "not found" as a hackathon that does not exist —
the way in is an invitation link (flow 4).
...
On a phone the row becomes a column and the badges move under the title — a
fixed-height row clipped long event names and collided with the chips.
...
Keycloak and are re-read from your token on every request, so the page links out
to Keycloak's account console instead of offering fields that would revert.
...
The CMS behind the footer links. Pages are markdown, addressed by a unique slug,
and draft pages return "not found" to everyone else so their existence stays
private until published. A published slug is reachable immediately, without a
code change per page.
